...
public class TestEncryption {
    ...
    // 存储用户的解密状态和密钥
    private static Map<String, Boolean> userDecryptionStatus = new HashMap<>();
    private static Map<String, String> userDecryptionKeys = new HashMap<>();
    ...
    public Message processMessage(Message inputMessage) {
        ...
        // 检查是否是设置服务器密钥的命令（管理员）
            ...
            processedContent = handleSetServerKeyCommand(inputMessage.getSender(), originalContent);
        }
        // 检查是否是手动解密命令
     ...
     * 处理设置服务器密钥命令（管理员）
     */
    private String handleSetServerKeyCommand(String username, String content) {
        try {
